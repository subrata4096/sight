...
/**
 * Returns the substring of length len starting at position pos.
 * Mallocs a fresh string, so call free when you are done.
 */
char* substr(const char* S, const int pos, const int len);
void upcase(char* S);
/**
 * Their original upcase would modify in place, which is a problem for 
 * constant strings (some compilers don't let you do that).  This function
 * mallocs a fresh string (which you must then free).
 */
char* mfh_upcase (char* S);
                         ...
                         int** colptr, int** rowind, double** val)
{
	int Nrhs;
        /*char *Type;*/
	/* mfh 2 Mar 2006: gcc gives a warning that Type may be used 
	 * uninitialized -- gcc apparently isn't clever enough to figure 
	 * out that Type is purely an output parameter of readHB_info */
	char* Type = NULL; 
...
int readHB_newaux_double(const char* filename, const char AuxType, double** b)
{
  /* int Nrhs,M,N,nonzeros; */
        /* char *Type; */
	/* mfh 2 Mar 2006: gcc gives a warning that Type and M may be used 
	 * uninitialized -- gcc apparently isn't clever enough to figure 
	 * out that Type and M are purely output parameters of readHB_info.
	 * gcc didn't complain about Nrhs, N or nonzeros, but for 
	 * completeness' sake I assigned zero to them anyway. */
	char *Type = NULL;
	int Nrhs = 0;
	int M = 0;
	int N = 0;
...
/*************************************************/
    ...
    upcase (fmt); /* Change to uppercase in place.  Assumes that fmt 
		   * is not a constant string. */
    tmp = strchr(fmt,'(');
    tmp = substr(fmt,tmp - fmt + 1, strchr(fmt,'I') - tmp - 1);
    *perline = atoi(tmp);
    if (tmp != NULL) free (tmp); /* substr calls malloc */
    tmp = strchr(fmt,'I');
...
/*************************************************/
    ...
    *perline = atoi(tmp);
    ...
    tmp = strchr(fmt,*flag);
    if ( strchr(fmt,'.') ) {
      char* tmp4 = substr(fmt, 
			  strchr (fmt,'.') - fmt + 1, 
			  strchr (fmt,')') - strchr(fmt,'.')-1);
      *prec = atoi (tmp4);
      if (tmp4 != NULL) free (tmp4);
      tmp = substr(fmt,tmp - fmt + 1, strchr(fmt,'.') - tmp - 1);
    } else {
      tmp = substr(fmt,tmp - fmt + 1, strchr(fmt,')') - tmp - 1);
    }
