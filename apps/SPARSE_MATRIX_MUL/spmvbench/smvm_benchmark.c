			     ...
			     struct SMVM_timing_results* p_results)
{
  ...
  smvm_timing_run_with_results2 (p_results, m_eff, n_eff, r, c, A->rowptr, 
                                 A->colind, (double*)A->values, num_nonzero_blocks,
...
smvm_benchmark (struct SMVM_parameters *p_params)
{
   ...
   *	values -> values
   */
  smvm_timing_run (m_eff, n_eff, r, c, A->rowptr, A->colind, (double*)A->values, 
   ...
   */
  if (b_verify)
    smvm_verify_result (m_eff, n_eff, r, c, A->rowptr, A->colind, (double*)A->values,
