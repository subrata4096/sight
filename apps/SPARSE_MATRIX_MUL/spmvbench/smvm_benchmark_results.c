...
{
  return (struct SMVM_benchmark_results*)smvm_malloc (sizeof (struct SMVM_benchmark_results));
}
  ...
  if (p->curlen >= p->maxlen)
    {
      ...
      p->array = (struct SMVM_benchmark_results**)smvm_realloc (p->array, p->maxlen * 
