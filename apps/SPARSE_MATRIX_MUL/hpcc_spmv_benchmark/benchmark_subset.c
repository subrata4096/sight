...
int max_problem_dimension(hpcc_spmv_params *bench_params)
{
  ...
  accum_time_ests = (double***)smvm_calloc(bench_params->numdims, sizeof(double**));

  for (logdim = 0; logdim < bench_params->numdims; logdim++) {
    accum_time_ests[logdim] = (double**)smvm_calloc(bench_params->n_row_blockdims, sizeof(double*));

    for (r = 0; r < bench_params->n_row_blockdims; r++) {
      accum_time_ests[logdim][r] = (double*)smvm_calloc(bench_params->n_col_blockdims, sizeof(double));
    }
  }
...
int benchmark_subset(hpcc_spmv_params *bench_params) {
  ...
  bench_params->threshold_dims = (int**)smvm_calloc(bench_params->n_row_blockdims,sizeof(int*));
  
  for (r = 0; r < bench_params->n_row_blockdims; r++) {
    bench_params->threshold_dims[r] = (int*)smvm_calloc(bench_params->n_col_blockdims,sizeof(int));
  }
  ...
  bench_params->nnz_row_vals = (int*)smvm_malloc(bench_params->nnz_per_row_to_run*sizeof(int));
