			       ...
			       const int upper_block_bandwidth)
{
  ...
  A = create_empty_bcoo_matrix (bm, bn, r, c, (index_base_t)0, UNSYMMETRIC, (symmetric_storage_location_t)0, PATTERN);
  ...
  while (num_iterations < MAX_NUM_ITERATIONS && A->nnzb < nnzb)
      ...
      if (smvm_debug_level() > 1)
	{
	  ...
	  //diff = (unsigned int) (A->JJ) - (unsigned int) (A->II);
	  diff = (int) *(A->JJ) - (int) *(A->II);

	  if (ABS(diff) < sizeof(int) * A->nnzb)
	    {
	      fprintf (stderr, "*** __create_random_matrix_banded: A->II and "
		       "A->JJ overlap by %u elements, after calling bcoo_matrix_coalesce_duplicate_entries ! ***\n", 
		       A->nnzb - diff/sizeof(int));
	      fprintf (stderr, "*** nnzb = %d, diff/sizeof(int) = %u ***\n", 
		       A->nnzb, diff/sizeof(int));
	      exit (EXIT_FAILURE);
	    }
	}
  ...
  if (smvm_debug_level() > 0)
	...
	{
	  fprintf (stderr, "*** __create_random_matrix_banded: A->II and "
		   "A->JJ overlap by %u elements! ***\n", 
		   A->nnzb - ABS(diff/sizeof(int)));
	  fprintf (stderr, "*** nnzb = %d, diff/sizeof(int) = %u ***\n", 
		   A->nnzb, diff/sizeof(int));
	  exit (EXIT_FAILURE);
	}
								 ...
								 const int upper_block_bandwidth)
{
  ...
  struct bcsr_matrix_t* A = (bcsr_matrix_t*)smvm_calloc (1, sizeof (struct bcsr_matrix_t));

  /* Vector for holding the column indices of the current row */
  int* current_row = (int*)smvm_calloc (nnzb_per_block_row, sizeof (int));
  ...
  A->symmetric_storage_location = (symmetric_storage_location_t)0;
  A->value_type = REAL;
  A->col_oriented_p = 0;
  ...
  for (bi = 0; bi < bm; bi++)
      ...
      while (num_added_to_this_row < nnzb_per_block_row)
	{
      ...
      for (int k = 0; k < nnzb_per_block_row; k++)
					       ...
					       const int upper_block_bandwidth)
{
  ...
  struct bcsr_matrix_t* A = (struct bcsr_matrix_t*)smvm_calloc (1, sizeof (struct bcsr_matrix_t));
  /* Just a guess for now */
					     ...
					     const double *interval_fracs)
{
  ...
  entries_per_decile = (int*)smvm_calloc(10, sizeof(int));
  colinds_to_add = (int*)smvm_calloc(nnzb_per_block_row, sizeof(int));
...
__create_random_matrix_banded_by_nnz_per_row (const int bm, const int bn, 
					      ...
					      const int upper_block_bandwidth)
{
  struct bcoo_matrix_t* A = create_empty_bcoo_matrix (bm, bn, r, c, (index_base_t)0, 
						      UNSYMMETRIC, (symmetric_storage_location_t)0, PATTERN);
  struct bcsr_matrix_t* B = NULL;
  /* Just a guess for now */
  const int MAX_NUM_ITERATIONS = 2 * nnzb_per_block_row * nnzb_per_block_row; 
