...
fptype CNDF ( fptype InputX ) 
{
    ...
    xInput = InputX;
    module cndfModule(instance("cndf", 1, 1),
                     inputs(port(context("xInput",  xInput,sight::common::module::notes(sight::common::module::publicized())))),
    ...
    cndfModule.setOutCtxt(0, context("OutputX", OutputX));
...
#pragma omp parallel for private(i, price, priceDelta)
        ...
        for (i=start; i<end; i++) {
             ...
             */
           {
            module blackScholsModule(instance("blackSchols", 1, 1),
                     inputs(port(context("sptprice", sptprice[i],
                                         "strike", strike[i], 
                                         "rate", rate[i], 
            ...
            blackScholsModule.setOutCtxt(0, context("prices", prices[i], "priceDelta", priceDelta));
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   if (argc != 4)
    ...
    if(getenv("NUM_RUNS")) NUM_RUNS = atoi(getenv("NUM_RUNS"));
    ...
    char* expId = "-1"; if(getenv("EXP_ID")) expId = getenv("EXP_ID");
        ...
        stringstream outDir;
#ifdef ENABLE_OPENMP
        outDir << "dbg.BLACKSCHOLES_OPENMP" << numOptions << "_" << nThreads << "_" << NUM_RUNS << "_" << expId;
#else
        outDir << "dbg.BLACKSCHOLES_SCALAR" << numOptions << "_" << nThreads << "_" << NUM_RUNS << "_" << expId;
#endif

        SightInit(argc, argv, "BLACKSCHOLES",txt()<< outDir.str());
...
#endif
{
  module totalBSModule(instance("totalBlackScholes", 1, 1),
                     inputs(port(context("numOptions",  numOptions,
                                         "nThreads", nThreads, 
                                         "NUM_RUNS", NUM_RUNS ))),getMeasures() );
#ifdef ENABLE_THREADS
#ifdef WIN32
    HANDLE *threads;
    int *nums;
    ...
    bs_thread(&tid);
     ...
     rssUsage = rss2 - rss1;
     vmUsage = vm2 - vm1;
     totalBSModule.setOutCtxt(0, context("rssUsage", rssUsage, "vmUsage", vmUsage));
}
